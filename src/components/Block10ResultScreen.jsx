...
function Block10ResultScreen() {
  const navigate = useNavigate()
  const { gameAnswers, calculateBlock10Score } = useGame()
  
  const score = calculateBlock10Score()
  const correctPhrase = 'Ela colocou café na xicára'

  const results = [
    {
      question: 'Organize a frase',
      userAnswer: gameAnswers.fraseOrdenada || 'Não respondido',
      correctAnswer: correctPhrase,
      isCorrect: gameAnswers.fraseOrdenada === correctPhrase
    }
  ]
  ...
  const getScoreMessage = () => {
    if (score === 1) return 'Excelente! Você organizou a frase corretamente!'
...
export default Block10ResultScreen
